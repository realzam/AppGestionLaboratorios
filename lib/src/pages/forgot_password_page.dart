...
class _ForgotPasswordPageState extends State<ForgotPasswordPage> {
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
            ...
            errorText: recoveryInfo.errorEmail),
        onChanged: (val) {
          recoveryInfo.errorEmail = null;
          recoveryInfo.email = val;
        },
      ),
    );
  }
  ...
  _register(BuildContext context, Function start, Function stop) async {
    bool res = recoveryInfo.validCorreo();
    if (res) {
      FocusScope.of(context).requestFocus(new FocusNode());
      start();
      Map<String, dynamic> response =
          await usuarioProvider.requetEmailPassword(recoveryInfo.email);
      if (!response['ok']) {
        stop();
